...
subset <- list(level = 925, time = "2000-01-01")
geopotential <- ReadNetCDF(
  "~/DATOS/NCEP Reanalysis/air.mon.mean.nc",
  c(t = "air"),
  subset = subset
)
geopotential[,
  u := ReadNetCDF(
    "~/DATOS/NCEP Reanalysis/uwnd.mon.mean.nc",
    c(u = "uwnd"),
    out = "vector",
    subset = subset
  )
...
geopotential[,
  v := ReadNetCDF(
    "~/DATOS/NCEP Reanalysis/vwnd.mon.mean.nc",
    c(v = "vwnd"),
    out = "vector",
    subset = subset
  )
...
geopotential %>%
  ...
  geom_streamline(
    aes(dx = dlon(u, lat), dy = dlat(v), size = after_stat(step)^2),
    # min.L = 1,
    dt = 36000 / 4,
    S = 40,
    lineend = "round",
