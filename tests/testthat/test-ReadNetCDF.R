...
test_that("returns a data.table", {
  expect_s3_class(ReadNetCDF(file), "data.table")
})
...
test_that("using I() works", {
  ...
  expect_equal(
    unique(ReadNetCDF(file, subset = list(level = I(1)))$level),
    1000
  )

  expect_equal(
    unique(ReadNetCDF(file, subset = list(level = I(1:3)))$level),
    c(1000, 925, 850)
  )
})
...
test_that("subsetting works", {
  r <- ReadNetCDF(file, subset = list(lat = -90:20))
...
test_that("naming works", {
  expect_true({
    t <- ReadNetCDF(file, vars = c(air2 = "air"))
    "air2" %in% colnames(t)
  })
})
...
test_that("function in vars works", {
  ...
  expect_identical(
    ReadNetCDF(file, vars = function(x) startsWith(x, "o3")),
    read_benchmark
  )
  expect_identical(
    ReadNetCDF(file, vars = function(x) x[startsWith(x, "o3")]),
    read_benchmark
  )

  expect_warning(expect_null(ReadNetCDF(file, vars = function(x) {
    startsWith(x, "o5")
  })))
...
test_that("multi-file works", {
  ...
  expect_equal(
    ReadNetCDF(
      OpenNetCDF(files),
      subset = list(time = c("1959-01-01", "1959-02-28"))
    ),
    ReadNetCDF(files, subset = list(time = c("1959-01-01", "1959-02-28")))
  )
})

# From https://github.com/eliocamp/metR/issues/217
test_that("parse era5 timestamp correctly", {
  file <- "era5_timestamp.nc"
  r <- ReadNetCDF(file)
