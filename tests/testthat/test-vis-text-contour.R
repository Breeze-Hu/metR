...
test_that("contour_text wokrs", {
  ...
  expect_doppelganger(
    "placement_minmax-vertical",
    ggplot(geo, aes(lon, lat)) +
      geom_contour(aes(z = gh)) +
      geom_text_contour(aes(z = gh), label.placer = label_placer_minmax())
  )

  expect_doppelganger(
    "placement_minmax-horizontal",
    ggplot(geo, aes(lon, lat)) +
      geom_contour(aes(z = gh)) +
      geom_text_contour(
        aes(z = gh),
  ...
  expect_doppelganger(
    ...
    ggplot(geo, aes(lon, lat)) +
      geom_contour(aes(z = gh)) +
      geom_text_contour(aes(z = gh), label.placer = label_placer_n(2))
  )
...
test_that("stroke.colour is aesthetic", {
  expect_doppelganger(
    ...
    ggplot(geo, aes(lon, lat)) +
      geom_contour(aes(z = gh)) +
      geom_text_contour(
        aes(z = gh, stroke.colour = after_stat(level)),
        stroke = 0.2
      )
  )
})
...
test_that("geom_label_contour also work", {
  expect_doppelganger(
    ...
    ggplot(geo, aes(lon, lat)) +
      geom_contour(aes(z = gh)) +
      geom_label_contour(
        aes(z = gh, fill = after_stat(level)),
