...
test_that("EOF, returns correct PCs", {
  expect_equal(
    nrow(EOF(gh ~ lat + lon | date, data = geopotential, n = 2)$sdev),
    1
  )
})
...
test_that("can use differnet engine", {
  ...
  expect_equal(
    EOF(gh ~ lat + lon | date, data = geopotential, engine = base::svd)$left[,
      gh := -gh
    ][],
...
test_that("EOF works inside data.table", {
  expect_equal(
    geopotential[, EOF(gh ~ lon + lat | date)$left],
...
test_that("EOF fails gracefully", {
  ...
  expect_error(
    EOF(gh ~ lon1 + lat2 | date, data = geopotential),
    "Columns not found in data: lon1, lat2"
  )
  expect_error(
    EOF(gh ~ lon | date, data = geopotential),
...
test_that("eof methods", {
  eof <- EOF(
    ...
    n = 1:5,
    engine = base::svd
  ) # need to force this engine so that predict is exact
  ...
  expect_equal(
    geopotential[, .(lat, lon, date, gh)][order(lat, lon, date)],
