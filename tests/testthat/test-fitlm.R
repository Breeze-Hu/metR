...
test_that("regression works", {
  ...
  sim_fit <- c(
    sim_fit,
    list(
      std.error = unname(summ$coefficients[, 2]),
      df = rep(summ$df[2], 3),
  ...
  expect_equal(ResidLm(x, y, z), unname(residuals(good_fit)))
})
...
test_that("weighted, regression works", {
  ...
  expect_equal(ResidLm(x, y, z, weights = weights), unname(residuals(good_fit)))
})
...
test_that("Supports NAs", {
  ...
  expect_equal(
    as.vector(na.omit(ResidLm(x, y, z))),
